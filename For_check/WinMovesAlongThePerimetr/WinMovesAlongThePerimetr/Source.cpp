...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	while (1)
	{
		...
		wsprintf(str, TEXT("Amount of wins: %d"), hList.size());

		if (MessageBox(0, TEXT("Reload?"), str, MB_YESNO | MB_ICONWARNING) == IDNO)
		{
			Restore();
			return 1;
		}
...
VOID CALLBACK TimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
	...
	for (auto it = hList.begin(); it != hList.end(); it++)
	{
		...
		{
		case 1:
			MoveWindow(hWindow, rect.left + step, rect.top, width, high, 1);
			break;
		case 2:
			MoveWindow(hWindow, rect.left, rect.top - step, width, high, 1);
			break;
		case 3:
			MoveWindow(hWindow, rect.left - step, rect.top, width, high, 1);
			break;
